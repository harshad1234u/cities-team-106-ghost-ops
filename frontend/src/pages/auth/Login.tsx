    ...
    try {
      ...
      if (signInError) {
        if (signInError.message.toLowerCase().includes('rate limit') || signInError.message.toLowerCase().includes('unconfirmed')) {
          ...
          if (email.includes('engineer') || email.includes('javithbasha')) role = 'engineer';
        ...
        let role = data.user.user_metadata?.role || localStorage.getItem(`civo_role_${data.user.id}`);
        
        if (!role) {
          if (email.includes('admin')) role = 'admin';
          else if (email.includes('engineer') || email.includes('javithbasha')) role = 'engineer';
          else role = 'citizen';
        }

        await api.syncUser(data.user.id, data.user.email || email, role);
    ...
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4 font-sans text-slate-100">
      <div className="w-full max-w-md bg-slate-950/80 border border-slate-800 rounded-3xl p-8 shadow-2xl backdrop-blur-md space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-black text-white tracking-tight">CivoAI Portal Login</h1>
          <p className="text-xs text-slate-400">Civic Road Safety & AI Infrastructure Platform</p>
        </div>
        ...
        <div className="pt-4 border-t border-slate-800/80 flex justify-between items-center text-xs text-slate-400">
          ...
          <Link to="/auth/signup" className="text-cyan-400 font-bold hover:underline">
            Register Here
          </Link>
        </div>
      </div>
